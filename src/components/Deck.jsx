...
const goTo = (i, total) => ({ x: 0, y: (total - 1 - i) * 4, scale: 1, rot: Math.random() * 20 - 10, delay: i * 100 });
...
export default function Deck({ images }) {
    ...
    const onMouseOverCard = (index) => {
        // Zoom on hover
        setSprings((i) => {
            if (i !== index) return;
            return { scale: 1.03, config: { friction: 50, tension: 500 } };
        });
    };

    // On mouse exiting the video -> Change cursor icon
    const onMouseOutCard = (index) => {
        // Zoom on hover
        setSprings((i) => {
            if (i !== index) return;
            return { scale: 1, config: { friction: 50, tension: 500 } };
        });
    };
    ...
    return (
        <div className="deck">
            {springProps.map(({ x, y, rot, scale }, i) => (
                <animated.div className="card" key={i} style={{ x, y }} onMouseOver={() => onMouseOverCard(i)} onMouseOut={() => onMouseOutCard(i)}>
